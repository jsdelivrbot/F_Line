	...
	<div>
		...
		<tabbar class="clinic_footer">
	      ...
	      <tabbar-item @click.native="gorating()">
	        ...
	        <!-- <img slot="icon-active" :src="textleft=='继续诊股'?marketact:''"> -->
...
export default {
    ...
    data(){
    	return{
			...
			// marketact:require('../../assets/clinic/diagnose_btn_optional_pre1@'+this.$store.state.dpr+'x.png'),
			...
			datas:'',
			addstate:''
			//traceimg:require('../../assets/clinic/diagnose_icon_trace@'+this.$store.state.dpr+'x.png'),
    	}
    },
     directives: {
	    TransferDom
	...
	methods:{
	   ...
	   //  	if(sessionStorage.userId!='undefined'){
			 ...
			 //    		this.market=require('../../assets/clinic/diagnose_btn_optional_pre1@'+this.$store.state.dpr+'x.png');
			 //    		this.addstate="已添加";
			 //    	}else{
			 //    		this.market=require('../../assets/clinic/diagnose_btn_optional_nor1@'+this.$store.state.dpr+'x.png');
			 //    		this.addstate="添加自选";
			 //    	}
			 //  	})
			 //  	.catch((error)=>{
	   ...
	   //  	}
		...
		// addmychoose(){
		// 	if(sessionStorage.userId!='undefined'&&this.addstate!='已添加'&&this.textright=='添加自选'){
		// 		let timestamp=(new Date()).valueOf();
		// 		let signhotdata=this.$socketsign(sessionStorage.userId+'&'+this.$route.query.code+'&'+timestamp);
		// 		//显示loading
