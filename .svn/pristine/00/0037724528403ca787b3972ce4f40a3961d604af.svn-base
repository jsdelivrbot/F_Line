     ...
     <div class="list1">
            ...
            <ul class="list-body" v-show="!show">
                <li class="list-detail" v-for="(item,index) in items" @click.stop.prevent="toIndividual(item.Code)">
                    ...
                    <div :class="[parseFloat(item.nowPrice) < 0 ? 'declearfont':'risefont' ]">{{parseFloat(item.nowPrice).toFixed(2)}}</div>
                    <div :class="[parseFloat(item.diff_rate) < 0 ? 'declearfont':'risefont' ]">{{parseFloat(item.diff_rate).toFixed(2)}}%</div>
                </li>
            </ul>
        </div>
</template>
